...
static void cpu_up(struct work_struct *work)
{
	int cpu;
...
static void cpu_down(struct work_struct *work)
{
	int cpu;
	int online = num_online_cpus();
	int nr_down = 1;
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	INIT_WORK(&dbs_info->up_work, cpu_up);
	INIT_WORK(&dbs_info->down_work, cpu_down);
